...
class Neo4jGraphStore:
    ...
    def __init__(self, path: str | Path | None = None) -> None:  # path unused
        ...
        auth = (os.environ.get("AGMEM_NEO4J_USER", "neo4j"),
                os.environ.get("AGMEM_NEO4J_PASSWORD", "neo4j"))
        self._driver = GraphDatabase.driver(uri, auth=auth)
        with self._driver.session() as s:
            s.run("CREATE CONSTRAINT agmem_entity_id IF NOT EXISTS"
    ...
    def upsert_node(self, node_id: str, namespace: str, name: str,
        ...
        self._run(
            ...
            id=node_id, ns=namespace, name=name, summary=summary,
            etype=entity_type, now=_now())
    ...
    def find_node_by_name(self, name: str, namespace: str) -> dict | None:
        rows = self._run(
            ...
            " n.created_at AS created_at", ns=namespace, name=name)
    ...
    def upsert_edge(self, edge_id: str, namespace: str, src: str, dst: str,
        ...
        self._run(
            ...
            src=src, dst=dst, eid=edge_id, ns=namespace, pred=predicate,
            content=content, valid=valid_at, now=_now())

    _EDGE_RETURN = (" RETURN e.id AS id, e.namespace AS namespace,"
    ...
    def edges_between(self, src: str, dst: str, namespace: str,
        ...
        return self._run(
            ...
            self._EDGE_RETURN, ns=namespace, src=src, dst=dst)
    ...
    def edges_for_nodes(self, node_ids: list[str], namespace: str,
        ...
        return self._run(
            ...
            active + self._EDGE_RETURN, ns=namespace, ids=node_ids)
