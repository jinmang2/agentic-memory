...
> - **#10 CLOSED** by the `rb_upstream` preset (`scripts/repro/configs.py`). The preset does not restate the operating point, it DERIVES `per_type_k` from `RB_READ_RECIPE` and expresses `strategies_topk: 0` as the type's absence from `memory_types`, so preset and constant cannot drift; a test pins both directions. It carries `run_ready=False`, and that is permanent rather than pending: RB's write path is task-trajectory-shaped (`add_task_result`), not conversation ingest, so no LoCoMo threading exists or is wanted — the arm exists so a future agent-bench run cites `--config rb_upstream` instead of hand-assembling the recipe (ledger C-4: code presence is not measurement). `default_memory_types` still serves episodic + experiences + strategies at k=10 for ordinary runs, which is unchanged and correct. The blocker recorded above was the shared-file one, not a decision — resolved by making the edit in a worktree, additive, with no existing arm touched.
