...
| **strengthen 액션** | `suggested_connections`를 **새 노트의 links에 추가**(단방향) + **`tags_to_update`로 새 노트의 tags 교체** (agiresearch `memory_system.py:679-683`, WujiangXu `memory_layer.py:832-836`) | `amem.py:139-145` connections → 새 노트 LINK + **역방향 LINK도 추가**(양방향). **새 노트 tags 갱신(`tags_to_update`) 미적용** — **[SUPERSEDED — 이 셀은 P1-5 수정 이전 코드의 기록. 현행 `organizers/amem/organizer.py`는 정반대: 링크 단방향("unidirectional, as upstream"), `new_note_tags`는 제2 UPDATE op로 적용(audit P1-5). 라운드 12 finding 5]** | **부분 누락 + 변형**(당시 판정; 현행 코드는 해소) | 중간: tags는 임베딩 텍스트 성분이므로 새 노트의 검색 표현이 진화하지 않음. 양방향 링크는 upstream에 없는 확장(1-hop 확장 구현 시 회수 범위가 넓어짐 — 명시할 것) |
...
| **주입 포맷** | **system 프롬프트**에 "Below are some memory items that I accumulated... please first explicitly discuss if you want to use each memory item or not, and then take action" + 아이템 markdown (`agents/legacy/agent.py:132-137`). 논문 App A.2: "each item represented by its **title and content**" (description 제외) | ~~`StrategyItem.render()` = title+description+content~~ **(정오: round-5 확인 — 실검색 경로는 `_DictItem` 렌더라 description이 소실됐었음. round-5 X3에서 description 렌더 추가로 수정. `StrategyItem.render()`는 read 경로 미사용.)** upstream 경로는 `WebArena/` 하위. MemoryBundle로 유저측 컨텍스트, 사용 지시문 없음 | **변형** | 낮음~중간 |
...
> **정오표 (2026-07-17, round-3 §1.4 A1 / round-4 재확인)**: 아래 표의 "#24 L2/cosine"
> 행은 이슈 두 건을 혼합 서술함 — "score 의미 반전(랭킹 유지)"은 **#23**, "L2 공간
> 생성(`hnsw:space` 미지정)"은 **#24**가 정확한 배정이다. 또한 "#10 silent skip" 행의
> silent skip은 이슈번호가 없는 광역 try/except이며 #10 자체는 add_note가
> analyze_content를 호출하지 않는 문제(현 main에도 잔존, open)다.
