...
### 3.1 규모 (o200k_base 실측)

| | `_s` (cleaned) | `oracle` |
|---|---|---|
| 인스턴스 | 500 | 500 |
| 세션/인스턴스 | p50 **48** (38–62) | p50 **2** (1–6) |
| 턴/인스턴스 | 평균 **494** | 평균 **21.9** |
| 렌더 토큰/인스턴스 | p50 **113,840**, 최대 114,991 | p50 **6,127**, 최대 23,770 |
| 전체 토큰 | **55.9M** | **3.06M** |
| 전체 턴 | **246,750** | 10,960 |
| chars/token | **4.610** | — |
| upstream 상한(126,200) 초과 | **0/500** | 0/500 |

`_s`는 LoCoMo 전체 캠페인(5,882턴)의 **42배** 물량이다. oracle의 948 세션은 `_s`의 evidence 세션 948과 정확히 일치한다(교차검증).
...
- 타입별 상대 개선: preference +77.7%, TR +48.2%, MS +16.7%, SSU +14.1%, **KU −3.36%, SSA −9.06%**. **두 타입에서 메모리가 손해**다 — 그리고 SSA 손해는 §2.3의 user-only 인덱싱 문제와 같은 뿌리일 가능성이 높다(어시스턴트 발화를 저장하지 않는 설계).
...
> **write 지출이 점수를 산다는 증거가 없다. 점수는 답변자에게 무엇을 건넸는가에서 난다.
> 그래서 LongMemEval 순위는 메모리 설계가 아니라 read 경로를 정렬한다 —
> V2가 reader를 고정하고 LAFS를 도입한 이유가 이것이다.**

부정형("증거가 없다")인 것이 의도적이다. 우리 표본은 5 arm·단일 시드라 **양의 주장을 지탱하지 못하고**,
부정형은 그 표본으로도 정확히 성립한다.
...
1. **n=5, 단일 시드.** ρ=−0.60도 +0.90도 유의하지 않다(n=5에서 p≈0.08~0.28). 그래서 주장은 반드시
   **"증거가 없다"** 형태여야 하고, "write는 무의미하다"로 강화하면 표본이 못 버틴다.
...
|---|---|---|
| 디스크 | 520 GB 여유 | 무관 |
| **RAM** | **총 7.9 GB, 가용 ~4.8 GB, swap 2 GB 사용 중** | **유일한 병목** |
| CPU | 4 코어 (런은 API 대기 지배) | 무관 |
| `oracle` 로드 | **peak RSS 72 MB / 1.1초** | ✅ 여유 |
| **`_s` 로드** | **peak RSS 2.42 GB / 35초** | ⚠️ 조건부 |
| 동시성 | `bench/locomo.py:843`의 ThreadPoolExecutor 패턴 재사용 | ✅ |
| 예상 wall-clock | oracle 500문항 × (생성 1 + judge 1), workers=8 → **런당 ~10분** | ✅ |

**`_s`를 돌릴 때의 3대 규칙** (안 지키면 OOM 또는 swap thrash):
① 프로세스가 아니라 **스레드**로 병렬화(메모리 공유) ② 인스턴스 **deep copy 금지**
(`prompt_rediff.py`가 하는 짓 — 감사 스크립트라 괜찮지만 드라이버에선 금지)
③ 로드한 원본 리스트를 필요 이상으로 붙들지 말 것.
**다른 유료 런과 `_s` 런을 병행하지 말 것** — 2.42 GB는 이 머신에서 병행 가능한 크기가 아니다.
...
- `_m`(2.74 GB) 미취득. `max_history_tokens`의 13% 과절단(D3)이 발동하는 유일한 변종이므로 받기 전에
  D3를 먼저 고칠 것
