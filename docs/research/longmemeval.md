...
탓에 독자는 7번째 타입(=헤드라인에서 제외된 것)으로 오독하기 쉽다. 원장 C-4의 표현은 교정 대상이다. → **교정 완료 (2026-08-19)**: C-4 Official code 셀에 날짜 있는 정정으로 반영.
...
| R4 | `_s` × luna × CoN | $12.02 | **$11.73** | task-avg **86.19** / overall 89.20 — P8 실측 확정: luna의 full-context 벌금은 −5.40pp(mini는 −23.20), 검색 없이 mini 최고 검색 arm(81.60)을 넘는다 (2026-08-20, docs/20 R4) |
...
| **C7** | **그리고 read 경로의 값은 코퍼스 길이에 상수가 아니다** | `_m`(476세션·4,894턴·1.11M토큰) vs `_s`(48·494·114K), 같은 500문항·같은 리더/프롬프트/judge/k=50·같은 배치 임베딩 체제: **81.40 → 72.80**, 페어링 **−8.60pp [−12.20,−5.00]**, task-avg −8.84, McNemar 68/25 p=9e-06. 낙폭은 균일하지 않다 — knowledge-update는 **+1.28 오르고**(85.90) single-session-assistant는 −1.78인데, preference −20.00·temporal-reasoning −15.79다. `_m`에는 비교할 full-context arm이 **존재할 수 없다**(중앙값이 128k 창의 8.8배) | **[실측]** |
...
미측정으로 남은 것(2026-08-20 갱신: `_s` × luna는 실측 완료 — R4, task-avg 86.19/overall 89.20/$11.73): §1.3의 **CoN 부호 역전**(`_s`에서 direct arm이 필요),
그리고 **LLM write를 쓰는 organizer × `_s`** — 최저가 Nemori도 ingest만 $50~70이라(턴 246,750개)
현 캡 밖이다. 전부 미승인.

### 9.4 이 주장의 약점 (먼저 적어 둔다 · 2026-08-18 재평가)

0. **측정한 체제 밖으로 일반화하지 말 것 — 그리고 이제 그 크기를 안다 (2026-08-19 갱신).**
   같은 검색 메커니즘이 oracle에서 **−3.00pp**, `_s`에서 **+21.20pp**다. **부호가 뒤집힌다.** 우리가
   oracle에서 먼저 잰 "검색세"를 그대로 밀었으면 정확히 틀린 결론을 발표할 뻔했다. 그래서 이 절의 모든
   문장에는 **어느 haystack 길이에서 잰 것인지**가 붙어야 하고, 붙지 않은 문장은 결함이다.

   ~~아직 안 잰 체제: `_m`~~ **`_m`을 쟀다(C7).** 같은 500문항·같은 read 경로에서 haystack만 9.9배로
   키우면 검색 점수가 **8.60pp 떨어진다**(81.40 → 72.80, 페어링 CI [+5.00,+12.20], McNemar 68/25,
   p=9e-06). ⇒ **C6의 +21.20pp 중 일부는 "검색의 값"이 아니라 "들어가는 haystack의 값"이었다.**
   C6는 죽지 않지만 **길이 조건이 붙는다**. 정본: [`docs/20`](../20-lme-reading.md).

   남은 일반화 구멍은 이제 **길이가 아니라 write 쪽**이다: `_m`에서 organizer arm은 $500~700이라
   (검색 arm은 ~$12) **write 경로가 존재하는 이유인 그 체제에서 정작 write 경로만 못 잰다.**
...
3c. 단, 그 값은 코퍼스 길이에 상수가 아니다              (C7 ← 실측: _s 81.40 → _m 72.80)
...
- ~~원장 C-4 갱신 3건 — §3.2(릴리스 계보), LME-A1/A2(SSA 91% 삭제), **§6 P2의 "이중계상" 표현 교정**~~
  **완료 (2026-08-19)**. 세 건 모두 `docs/17` C-4에 날짜 표기로 반영됐다 — §3.2는 Impact 셀의
  2026-08-07 노트에 대한 정제(순수 삭제·oracle sha 동일·"하한" 진술)로, LME-A1/A2는 Official code
  셀의 신규 단락으로, P2는 "이중계상" 문장의 날짜 있는 정정으로. 같은 날 C-4의 2026-08-05
  "NOT wired and NEVER RUN" 상태 블록도 supersession 행으로 갱신됐다(docs/20의 실측 arm들)
...
- ~~`_s` × luna 미측정~~ **완료 (2026-08-20, R4)**: task-avg 86.19 / overall 89.20, $11.73 — P8 확정, docs/20 R4. **`_s` × direct는 여전히 미측정**(§1.3 CoN 부호 역전이 여기 있다)
- ~~`_m` 검색 arm 미측정~~ **완료 (2026-08-19, C7)**. 남은 `_m` 구멍은 write 쪽뿐이고 $500~700이다
- ~~검색 recall을 아티팩트에서 복원할 수 없다~~ — 두 반쪽으로 갈라 둘 다 닫혔다 (2026-08-19).
  ① **캡처 반쪽 완료** (`f9f2ed3`): `retrieval_provenance`가 hit을 그 뒤의 세션 id들로 해석해 행에
  싣고, 행마다 `answer_session_ids` 대비 `evidence_recall`을 채점한다. organizer 메모리(여러 세션의
  요약, 혹은 세션 없음)는 0.0이 아니라 `null`로 보고된다 — 모양이 다르다는 이유로 recall이 0으로
  끌리지 않게. 비용이 붙기 전에 고쳤으므로 공짜였다는 원래 논리 그대로다.
  ② **렌더링된 프롬프트 기준 recall도 완료** (2026-08-19, 별도 작업 라인): 검색된 목록이 아니라
  리더에게 실제로 도달한 프롬프트 기준의 recall. 이로써 C7의 8.60pp를 검색 실패와 리더 실패로
  가르는 데 필요한 캡처는 다음 arm부터 전부 존재한다 — 단, 이미 지불된 arm들의 행은 소급되지 않는다
- **organizer × `_s` 미측정 — 배선·견적 완료, 실행은 보류 결정 (2026-08-19)**. 여전히 남은 가장 큰
  구멍이고, 우리 LME 수치에 organizer가 들어간 적은 없다. 달라진 것은 숫자가 실측이 됐다는 것:
  **~~Nemori $50~70~~ → 실측 $0.188/인스턴스**(3문항 스모크, write 콜 ~984/인스턴스) ⇒ 500문항
  **~$94 / 26시간**, 150문항 ~$28 / 8시간. **A-Mem ~$110 · Zep ~$210도 같은 22.5배 환산의 산물이라
  같이 신뢰할 수 없다.**

  **돌리지 않기로 한 이유**(정본: [`docs/20`](../20-lme-reading.md)): ① 현 배선으로는 답이 안 나온다 —
  `k`가 타입별로 적용돼 organizer arm이 컨텍스트를 1.14~1.54배 더 받고, 이 캠페인은 컨텍스트 양이
  점수를 크게 움직인다는 걸 이미 실측했다(C6) ② 같은 방향의 null이 이미 넷이다(ACE·RB·Zep §4.1·C1의
  ρ=−0.60) ③ 감당 가능한 n=150의 페어링 CI가 ±6~7pp라 null이어도 큰 효과만 배제한다.
  ⇒ **C6이 깔아둔 검색 바닥 위에 LLM write가 무엇을 더하는지는 여전히 미측정이고, 그 상태를
  선택했다.** 재개하려면 읽기 예산 정렬이 선행 조건이다
- ~~ingest 임베딩이 턴당 1 왕복이다~~ **해결 (2026-08-19, `bulk_ingest`)** — `_s` 4.3시간 → 11.6분(22배),
  `_m` 20시간 → ~2.5시간. 단 **"같은 벡터"는 틀렸다**: 배치 임베딩은 비트 동일이 아니고(cosine
  0.999999546) 점수는 안 움직이지만(+0.20pp) **개별 판정 39/500을 뒤집는다**. 그래서 배치 arm은
  배치 arm끼리만 비교하고 `embed_batch`를 스탬프에 남긴다(docs/20)
- **워커는 TPM 버스트로 묶인다** — `_m`에서 workers=8은 429 대기 563회 + 행 유실 9건, workers=6은
  195회 + 0건. 평균 처리량은 5M TPM 아래인데도 배치 128개가 몰려 나가 천장을 친다. 다음 arm 기본값은 6
