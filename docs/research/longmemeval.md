...
- **검색 recall을 아티팩트에서 복원할 수 없다** — 행이 검색된 **episode id**만 담고 그 뒤의 세션 id를
  담지 않아, C7의 8.60pp를 **검색 실패와 리더 실패로 가를 수 없다**. 사전등록에 미리 명시했고(docs/20),
  닫으려면 캡처 변경 + 재실행이 필요하다. 다음 arm 전에 고칠 것 — 비용이 붙기 전에 고치면 공짜다
