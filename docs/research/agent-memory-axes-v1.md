...
> **시점 안내 (2026-09-08)**: 이 문서의 코드 공백·제품 조사는 v1 착수 전 스냅샷이다. 원문 보존·세션 배선·탐색기·프로젝트 게이팅 등은
> 이후 구현되었으므로 §2·§7의 공백을 현재 TODO로 사용하지 않는다. 현재 상태는 [v1 정본](../23-v1-experience-memory.md),
> 새로 대조한 구현·측정·연구 후보는 [후속 조사](../24-next-research-and-implementation.md)를 따른다. 날짜가 있는 관찰은 역사 기록으로 보존한다.
