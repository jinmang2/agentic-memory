...
→ **완료 (후속 라운드)**: 조회 통지 훅 `Organizer.on_retrieval` 신설(MemoryOS N_visit 피드백, G-Memory served-insight 캐시); G-Memory 점수 의미론(ADD 2/EDIT·AGREE +1/soft REMOVE −1·−3/score≤0 프루닝, backward 폐루프); Zep 재구축(3단계 resolution: 임베딩 후보 0.6→exact-name→LLM 판정+name/summary 갱신, previous-episodes n=4 컨텍스트, temporal 통합 추출(valid_at/invalid_at), same-pair dup+contradiction 단일 콜+시간중첩 가드, name-only 임베딩); read: facts/entities BM25(items FTS)+dense RRF, GraphRecall(entity→incident active edges); RB 구-P3(경험 단위 검색+pipeline 확장, judge 이유 첨부, SI system 메시지+persona, content 1-3문장, 절단 60k). 잔여: Zep community(label propagation), G-Memory MAS 채널 분리·FINCH, RB MaTTS — 각 벤치 착수 시.
**실 스택 추가 배선**: graph_store 슬롯 신설 — KuzuGraphStore(임베디드 실 그래프 엔진, lite/standard 기본)·Neo4jGraphStore(서비스 감지 시, full 기본); doc_store 슬롯 신설 — PostgresDocStore(pgserver 임베디드 실 PostgreSQL+tsvector, full 기본; nemori config는 Postgres+Qdrant 듀얼로 upstream 스택 재현).
**P-doc**: docs/10 각 행 교정(본 문서 §2의 라벨 정정·누락 확장 반영), deep-audit §3 주입 포맷 행 교정, memoryos/sqlite_graph docstring 갱신. → 완료.
