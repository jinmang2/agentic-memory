...
Both are generated or executed, never hand-written: the first is emitted by
`scripts/repro/demo_cost_is_tokens.py` from `results/repro/`, the second is the actual output of
`scripts/repro/defects/run_all.py`, which CI runs on every push.
