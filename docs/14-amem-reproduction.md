...
metrics(per-category + overall) + cost(role별 llm_calls/tokens_in/out)가 담긴다. per-question
감사 추적(`<tag>.records.jsonl`)과 실행 로그는 아래 "산출물 & 영속성" 참조.
...
### 산출물 & 영속성 (Artifacts & persistence)

한 번의 실행은 세 가지 산출물을 남기며, **셋 다 git-tracked**라 실행 기록이 휘발되지
않는다 ("nothing is lost"):

| 산출물 | 경로 | 내용 |
|---|---|---|
| 요약 JSON | `results/repro/<tag>.json` | config stamp + per-category/overall metrics + cost. lean하게 유지되며, 감사 추적은 `records_file` 필드로 아래 sidecar를 가리킨다. |
| per-question 감사 추적 | `results/repro/<tag>.records.jsonl` | 질문 1개당 JSON 1줄(JSONL, appendable/greppable). 각 줄은 `run`·`conv` 태그 + `evaluate()` 레코드 필드(`q`/`gold`/`pred`/`cat`/`f1`, judge 시 `j`)를 담는다. **모든 conv·모든 run·모든 질문**이 기록된다 (cat5는 해석된 MCQ 답이 `pred`). `--ingest-only`(eval 없음)면 records 파일은 생성되지 않는다. |
| 실행 로그 | `results/repro/logs/<script>_<UTC-timestamp>.log` | `scripts/repro/*.sh`가 `tee`로 콘솔+파일에 동시 기록. `.gitignore`의 전역 `*.log`를 `!results/repro/logs/*.log` un-ignore로 덮어 **로그도 커밋**된다. |

`<tag>` = `<model>_<conv>_k<k>_<eval-mode>_expand-<on|off>_run<runs>`.
